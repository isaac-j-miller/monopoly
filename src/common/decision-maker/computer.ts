...
export class ComputerDecisionMaker extends DecisionMakerBase implements IDecisionMaker {
    async doOptionalActions(): Promise<void> {
        ...
        const allPropertyProperties = this.game.state.board.getAllPositionsOfType(PositionType.Property).filter(p=>properties.has(p.propertyId));
        allPropertyProperties.forEach(property => {
            const {color } = property;
            const propertiesOfColor = this.game.state.board.getAllPropertiesOfColor(color).map(p=>p.propertyId);
    ...
    async decideToAcceptTransferLoanQuote(quote: TransferLoanQuote, coverShortfallRound?: number): Promise<boolean> {
       if(quote.creditor === this.player.id) {
            // we are selling one of our own loans;
            const [_ratingAdjustedValue, faceValue] = this.getValueOfLoanToPlayer(quote.loanId, coverShortfallRound);
    ...
    async coverCashOnHandShortfall(): Promise<void> {
        ...
        await this.player.handleFinanceOption((this.player.cashOnHand * -1) + 1, "negative cash on hand balance")
        let round = 0;
    ...
    async decideHowToFinancePayment(amount: number): Promise<LoanQuote | null> {
        if(this.player.cashOnHand < amount || (this.player.creditRating >= CreditRating.AA && amount > this.player.cashOnHand * 0.5)) {
    ...
    private getValueOfLoanToPlayer(loanId: LoanId, coverShortfallRound: number = 0): [number, number] {
        ...
        const ratingAdjustedValue = faceValue * this.config.credit.ratingMultiplierOnDebtAssetValue[debtorCreditRating];
    ...
    async getLoanQuoteForPlayer(playerId: PlayerId, amount: number): Promise<LoanQuote | null> {
        ...
        if(this.player.cashOnHand < amount*2) {
            ...
            const bestLoanOffer = loanOffers.sort((a,b)=>getLoanQuoteFaceValue(a)-getLoanQuoteFaceValue(b))[0];
            if(!bestLoanOffer) {
                return null;
            }
