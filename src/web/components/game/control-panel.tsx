  ...
  return (
    <Card>
      <VerticalDiv>
        ...
        <p>Cash on Hand: ${currencyFormatter(player.cashOnHand)}</p>
        <p>Credit Rating: {CreditRating[player.creditRating]}</p>
        <p>Net Worth: ${currencyFormatter(player.getNetWorth())}</p>
        <p>Total Assets: ${currencyFormatter(player.getTotalAssetValue())}</p>
        <p>Total Liabilities: ${currencyFormatter(player.getTotalLiabilityValue())}</p>
        {taskComponent}
      </VerticalDiv>
    </Card>
  );
